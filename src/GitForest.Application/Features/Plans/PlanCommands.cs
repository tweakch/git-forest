...
internal sealed class InstallPlanHandler : IRequestHandler<InstallPlanCommand, InstalledPlanResult>
{
    ...
    public async Task<InstalledPlanResult> Handle(InstallPlanCommand request, CancellationToken cancellationToken)
    {
...
internal sealed class ReconcilePlanHandler : IRequestHandler<ReconcilePlanCommand, ReconcileResult>
{
    ...
    public async Task<ReconcileResult> Handle(ReconcilePlanCommand request, CancellationToken cancellationToken)
    {
        if (request is null) throw new ArgumentNullException(nameof(request));
