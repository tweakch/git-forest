...
internal static class ForestStore
{
        ...
        if (!File.Exists(configYamlPath))
        {
            File.WriteAllText(
                ...
                $"# Repo-level git-forest config{Environment.NewLine}"
                    ...
                    + $"  clusterId: {ForestConfigReader.DefaultOrleansClusterId}{Environment.NewLine}"
                    + $"  serviceId: {ForestConfigReader.DefaultOrleansServiceId}{Environment.NewLine}"
                    + $"  gatewayHost: {ForestConfigReader.DefaultOrleansGatewayHost}{Environment.NewLine}"
                    + $"  gatewayPort: {ForestConfigReader.DefaultOrleansGatewayPort}{Environment.NewLine}"
    ...
    public static ReconcileResult ReconcilePlan(string forestDir, string planId, bool dryRun)
    {
        ...
        var planters = plan.Planters.Count > 0 ? plan.Planters : Array.Empty<string>();
        ...
        for (var i = 0; i < templates.Count; i++)
        {
            ...
            var assignedPlanters =
                planters.Count > 0 ? new[] { planters[0] } : Array.Empty<string>();
