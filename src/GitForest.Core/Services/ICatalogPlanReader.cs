...
public interface ICatalogPlanReader
{
    ...
    /// </summary>
    Task<IReadOnlyList<CatalogPlan>> ListCatalogPlansAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets a specific plan from the catalog by its ID.
    /// </summary>
    Task<CatalogPlan?> GetCatalogPlanByIdAsync(string planId, CancellationToken cancellationToken = default);
}
