...
/// </summary>
public sealed class OrleansPlannerRepository
    : AbstractRepositoryWithSpecs<Planner, string>,
        IPlannerRepository
{
    ...
    public override async Task<Planner?> GetByIdAsync(
        string id,
    ...
    )
    {
        ...
        if (string.IsNullOrWhiteSpace(entity.Id))
            throw new ArgumentException("Planner.Id must be provided.", nameof(entity));
    ...
    protected override Task<IReadOnlyList<Planner>> LoadAllAsync(
        CancellationToken cancellationToken = default
    ) => GetAllAsync(cancellationToken);

    private async Task<IReadOnlyList<Planner>> GetAllAsync(
        CancellationToken cancellationToken = default
    )
    {
        var indexGrain = _grainFactory.GetGrain<IPlannerIndexGrain>(0);
