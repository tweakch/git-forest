...
internal static class PlantFileMapper
{
    public static PlantFileModel ToFileModelAndSyncDomain(Plant plant)
    {
        ...
        var updated = plant.LastActivityDate.HasValue
            ? new DateTimeOffset(
                DateTime.SpecifyKind(plant.LastActivityDate.Value, DateTimeKind.Utc)
            )
            : (DateTimeOffset?)null;
    ...
    public static Plant ToDomain(PlantFileModel model)
    {
        ...
        return new Plant
        {
            ...
            AssignedPlanters = (model.AssignedPlanters ?? Array.Empty<string>())
                .Where(x => !string.IsNullOrWhiteSpace(x))
                .Select(x => x.Trim())
                .ToList(),
            Branches = (model.Branches ?? Array.Empty<string>())
                .Where(x => !string.IsNullOrWhiteSpace(x))
                .Select(x => x.Trim())
                .ToList(),
            CreatedDate = created,
