...
public sealed class HandlerAndRepositoryTests
{
    ...
    public async Task ListPlants_returns_all_plants_sorted_by_key()
    {
        ...
        var result = await handler.Handle(
            new ListPlantsQuery(Status: null, PlanId: null),
    ...
    public async Task ListPlants_filters_by_status_and_sorts()
    {
        ...
        var result = await handler.Handle(
            new ListPlantsQuery(Status: "planned", PlanId: null),
            CancellationToken.None
        );
    ...
    public async Task ListPlants_filters_by_planId_and_status()
    {
        ...
        var result = await handler.Handle(
            new ListPlantsQuery(Status: "planned", PlanId: "p1"),
            CancellationToken.None
        );
    ...
    public async Task GetPlantByKey_returns_matching_plant()
    {
        ...
        var plant = await handler.Handle(
            new GetPlantByKeyQuery("  plan:slug  "),
            CancellationToken.None
        );
    ...
    {
        var repo = new InMemoryPlanRepository(
            new[]
            {
    ...
    public async Task ListPlanters_includes_builtin_aggregated_from_plans()
    {
        var plans = new InMemoryPlanRepository(
            new[]
            {
                ...
                {
                    Id = "p1",
                    Planters = new List<string> { "alpha", "beta" },
                },
                new Plan
                {
                    Id = "p2",
                    Planters = new List<string> { "alpha" },
                },
            }
        );
        ...
        var rows = await handler.Handle(
            new ListPlantersQuery(IncludeBuiltin: true, IncludeCustom: false),
            CancellationToken.None
        );

        Assert.That(
            rows.Select(r => $"{r.Id}:{r.Kind}:{string.Join(",", r.Plans)}"),
            Is.EqualTo(new[] { "alpha:builtin:p1,p2", "beta:builtin:p1" })
        );
    }
    ...
    public async Task ListPlanners_aggregates_from_plans_and_supports_plan_filter()
    {
        var plans = new InMemoryPlanRepository(
            new[]
            {
                ...
                {
                    Id = "p1",
                    Planners = new List<string> { "x", "y" },
                },
                new Plan
                {
                    Id = "p2",
                    Planners = new List<string> { "x" },
                },
            }
        );
        ...
        var all = await handler.Handle(
            new ListPlannersQuery(PlanFilter: null),
            CancellationToken.None
        );
        Assert.That(
            all.Select(r => $"{r.Id}:{string.Join(",", r.Plans)}"),
            Is.EqualTo(new[] { "x:p1,p2", "y:p1" })
        );

        var filtered = await handler.Handle(
            new ListPlannersQuery(PlanFilter: "p2"),
            CancellationToken.None
        );
        Assert.That(
            filtered.Select(r => $"{r.Id}:{string.Join(",", r.Plans)}"),
            Is.EqualTo(new[] { "x:p2" })
        );
    }
    ...
    public async Task AssignPlanter_transitions_planned_to_planted_and_persists_when_not_dry_run()
    {
        ...
        var updated = await handler.Handle(
            new AssignPlanterToPlantCommand(Selector: "p:a", PlanterId: "worker", DryRun: false),
            CancellationToken.None
        );
    ...
    public async Task AssignPlanter_dry_run_does_not_persist_changes()
    {
        ...
        var updated = await handler.Handle(
            new AssignPlanterToPlantCommand(Selector: "p:a", PlanterId: "worker", DryRun: true),
            CancellationToken.None
        );
    ...
    public async Task UnassignPlanter_removes_planter_and_persists_when_not_dry_run()
    {
        var repo = new InMemoryPlantRepository(
            new[]
            {
                new Plant
                {
                    ...
                    PlanId = "p",
                    Status = "planted",
                    AssignedPlanters = new List<string> { "worker", "other" },
                },
            }
        );
        ...
        var updated = await handler.Handle(
            new UnassignPlanterFromPlantCommand(
                Selector: "p:a",
                PlanterId: "worker",
                DryRun: false
            ),
        ...
        Assert.That(
            async () =>
                await handler.Handle(
                    new HarvestPlantCommand(Selector: "p:a", Force: false, DryRun: false),
                    CancellationToken.None
                ),
            Throws.TypeOf<InvalidOperationException>()
        );
    ...
    public async Task Archive_requires_harvested_unless_force()
    {
        ...
        Assert.That(
            async () =>
                await handler.Handle(
                    new ArchivePlantCommand(Selector: "p:a", Force: false, DryRun: false),
                    CancellationToken.None
                ),
            Throws.TypeOf<InvalidOperationException>()
        );

        var forced = await handler.Handle(
            new ArchivePlantCommand(Selector: "p:a", Force: true, DryRun: false),
            CancellationToken.None
        );
    ...
    public async Task Selector_PIndex_resolves_deterministically_by_key_order()
    {
        ...
        var updated = await handler.Handle(
            new AssignPlanterToPlantCommand(Selector: "P01", PlanterId: "worker", DryRun: true),
            CancellationToken.None
        );
    ...
    public async Task ForumPlanReconciler_calls_forum_and_creates_normalized_plants_deterministically_and_idempotently()
    {
        ...
        var (pid1, created1, updated1) = await reconciler.ReconcileAsync(
            planId,
            dryRun: false,
            forum: null,
        ...
        var (pid2, created2, updated2) = await reconciler.ReconcileAsync(
            planId,
            dryRun: false,
            forum: null,
            cancellationToken: CancellationToken.None
        ...
        Assert.That(
            all.Select(x => x.Key).OrderBy(x => x, StringComparer.Ordinal).ToArray(),
    ...
    public async Task ForumPlanReconciler_updates_plan_owned_fields_but_preserves_status()
    {
        ...
        var forum = new StaticForum(
            new ReconciliationStrategy(
                new[]
                {
                    new DesiredPlant(
                        Key: "p:keep-status",
        ...
        var (_, created, updated) = await reconciler.ReconcileAsync(
            planId,
            dryRun: false,
            forum: null,
            cancellationToken: CancellationToken.None
        ...
        Assert.That(
            persisted!.Status,
            Is.EqualTo("archived"),
            "Status should be preserved across reconcile"
        );
        ...
        Assert.That(
            persisted.Branches,
            Is.EqualTo(new[] { "some/branch" }),
            "Branches should be preserved across reconcile"
        );
    }
    ...
    private sealed class FlippingOrderForum : IReconciliationForum
    {
        ...
        )
        {
            ...
            var list = CallCount % 2 == 1 ? _desired.Reverse().ToList() : _desired.ToList();
