...
public sealed class ForestInitAndPlantsListTests
{
    ...
    public async Task Init_creates_required_git_forest_structure_and_plants_list_references_plan_planter_and_planner()
    {
        ...
        try
        {
            ...
            // 1) init
            var init = await GitForestCli.RunAsync(workingRepoDir, ["init"], timeout: TimeSpan.FromMinutes(3));
            ...
            // 2) plans install
            var install = await GitForestCli.RunAsync(workingRepoDir, ["plans", "install", planSource], timeout: TimeSpan.FromMinutes(3));
            ...
            var reconcile = await GitForestCli.RunAsync(workingRepoDir, ["plan", "integration-testing-harness", "reconcile"], timeout: TimeSpan.FromMinutes(3));
            ...
            var reconcile2 = await GitForestCli.RunAsync(workingRepoDir, ["plan", "integration-testing-harness", "reconcile"], timeout: TimeSpan.FromMinutes(3));
            ...
            var list = await GitForestCli.RunAsync(workingRepoDir, ["plants", "list"], timeout: TimeSpan.FromMinutes(1));
            ...
            var plantersJson = await GitForestCli.RunAsync(workingRepoDir, ["planters", "list", "--json"], timeout: TimeSpan.FromMinutes(1));
            ...
            using (var plantersDoc = JsonDocument.Parse(plantersJson.StdOut.Trim()))
            {
                ...
                Assert.That(plantersArray.GetArrayLength(), Is.GreaterThan(0), "Expected at least one planter after installing the plan");
            ...
            var plannersJson = await GitForestCli.RunAsync(workingRepoDir, ["planners", "list", "--json"], timeout: TimeSpan.FromMinutes(1));
            ...
            using (var plannersDoc = JsonDocument.Parse(plannersJson.StdOut.Trim()))
            {
                ...
                Assert.That(planners.GetArrayLength(), Is.GreaterThan(0), "Expected at least one planner after installing the plan");
            ...
            var listJson = await GitForestCli.RunAsync(workingRepoDir, ["plants", "list", "--json"], timeout: TimeSpan.FromMinutes(1));
    ...
    public async Task Plant_can_move_through_full_lifecycle_and_creates_branch_and_commit()
    {
        ...
        try
        {
            ...
            Assert.That((await GitForestCli.RunAsync(workingRepoDir, ["init"], timeout: TimeSpan.FromMinutes(3))).ExitCode, Is.EqualTo(0));
            Assert.That((await GitForestCli.RunAsync(workingRepoDir, ["plans", "install", planSource], timeout: TimeSpan.FromMinutes(3))).ExitCode, Is.EqualTo(0));
            Assert.That((await GitForestCli.RunAsync(workingRepoDir, ["plan", "integration-testing-harness", "reconcile"], timeout: TimeSpan.FromMinutes(3))).ExitCode, Is.EqualTo(0));

            var plantKey = "integration-testing-harness:add-integration-tests";
            var planterId = "harness-builder";

            // planned -> planted (assign)
            var assign = await GitForestCli.RunAsync(workingRepoDir, ["plant", plantKey, "assign", planterId], timeout: TimeSpan.FromMinutes(1));
            Assert.That(assign.ExitCode, Is.EqualTo(0), () => $"assign failed\nSTDOUT:\n{assign.StdOut}\nSTDERR:\n{assign.StdErr}");

            // planter plant (branch + record branch)
            var plant = await GitForestCli.RunAsync(workingRepoDir, ["planter", planterId, "plant", plantKey, "--branch", "auto", "--yes"], timeout: TimeSpan.FromMinutes(2));
            ...
            var grow = await GitForestCli.RunAsync(workingRepoDir, ["planter", planterId, "grow", plantKey, "--mode", "apply"], timeout: TimeSpan.FromMinutes(2));
            ...
            var harvest = await GitForestCli.RunAsync(workingRepoDir, ["plant", plantKey, "harvest"], timeout: TimeSpan.FromMinutes(1));
            Assert.That(harvest.ExitCode, Is.EqualTo(0), () => $"harvest failed\nSTDOUT:\n{harvest.StdOut}\nSTDERR:\n{harvest.StdErr}");

            // harvested -> archived
            var archive = await GitForestCli.RunAsync(workingRepoDir, ["plant", plantKey, "archive"], timeout: TimeSpan.FromMinutes(1));
            Assert.That(archive.ExitCode, Is.EqualTo(0), () => $"archive failed\nSTDOUT:\n{archive.StdOut}\nSTDERR:\n{archive.StdErr}");

            // status should count archived >= 1
            var status = await GitForestCli.RunAsync(workingRepoDir, ["status", "--json"], timeout: TimeSpan.FromMinutes(1));
